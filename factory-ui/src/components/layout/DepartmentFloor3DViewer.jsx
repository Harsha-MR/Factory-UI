...
export default function DepartmentFloor3DViewer({
  ...
  useEffect(() => {
    ...
    // Enable damping for smooth, inertial camera movement (improves feel without rendering cost)
    if ("enableDamping" in controls) controls.enableDamping = true;
    // Higher damping in preview = smoother deceleration for better feel
    if ("dampingFactor" in controls)
      controls.dampingFactor = fullScreen ? 0.12 : 0.18;

    // Make movement speeds responsive
    if ("rotateSpeed" in controls) controls.rotateSpeed = 1.0;
    if ("zoomSpeed" in controls) controls.zoomSpeed = 1.2; // Faster zoom response for snappier feel
  ...
  return (
    <div
      ...
      style={
        fullScreen
          ? { height: "100vh", minHeight: "100vh" } // ✅ FIX
    ...
    >
      ...
      >
        <Canvas
          ...
          // Slightly higher DPR in preview for better responsiveness without heavy drain
          dpr={[1, 1.25]}
          gl={{ antialias: false, powerPreference: "high-performance" }}
          // Keep on-demand rendering for battery efficiency
