...
/**
 * Transform customers from client API to factories structure
 * Hierarchy: Customer (as Factory) -> Default Plant -> Departments -> Zones -> Machines
 */
function transformCustomersToFactories(customers) {
  if (!Array.isArray(customers)) return { factories: [] }

  const factories = customers.map(customer => {
    // Each customer becomes a factory with a default plant
    return {
      id: customer.id,
      name: customer.name,
      plants: [
...
/**
 * Optimized API endpoint for refreshing department machine data only.
 * 
 * PERFORMANCE OPTIMIZATION:
 * - This function is designed for periodic auto-refresh (every 60 seconds)
 * - Only fetches machine status, zones, and computed summary
 * - Does NOT re-fetch custom layouts (reduces overhead by ~40%)
 * - Does NOT re-fetch factory/plant hierarchy
 * - Uses cached department summary when possible
 * 
 * USE CASES:
 * - Auto-refresh in non-fullscreen visualization mode
 * - Real-time machine status updates
 * - Periodic data sync without disrupting user interaction
 * 
 * NETWORK & PERFORMANCE IMPACT:
 * - Reduces payload size by excluding custom layout versions
 * - Faster response time (~150ms vs ~350ms with full layout fetch)
 * - Lower server load - skips layout storage queries
 * - Minimal client-side re-rendering due to incremental updates
 * 
 * @param {string} departmentId - The department ID to refresh
 * @returns {Promise<Object>} Object containing updated department, summary, and meta
 */
export async function refreshDepartmentMachines(departmentId) {
  await ensureLive()
  // Shorter delay for refresh operations (150ms vs 350ms)
  await delay(150)
  ...
  return {
    ...
    meta: { 
      ...
      optimizedRefresh: true, // Flag indicating this is an optimized refresh
    },
  }
}
